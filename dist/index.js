...
function createDeck() {
    const cards = [];
    for (let i = 0; i < 5; i++)
        cards.push({ name: "Guard", value: 1 });
    for (let i = 0; i < 2; i++)
        cards.push({ name: "Priest", value: 2 });
    for (let i = 0; i < 2; i++)
        cards.push({ name: "Baron", value: 3 });
    for (let i = 0; i < 2; i++)
        cards.push({ name: "Handmaid", value: 4 });
    for (let i = 0; i < 2; i++)
        cards.push({ name: "Prince", value: 5 });
...
// Supabase helpers
async function upsertGameState(code, state) {
    // Upsert by code
    const { data, error } = await supabase
        ...
        .limit(1);
    if (error)
        console.error("upsertGameState error", error);
...
function determineWinnerByHands(players) {
...
function nextTurnServerSide(state) {
    ...
    // expire protection
    if (state.players[state.currentPlayerIndex].protected)
        delete state.players[state.currentPlayerIndex].protected;
...
function applyCardEffect(state, playerIndex, card, targetSocketId, guessedCard, socket) {
...
function findPlayerIndexBySocket(state, socketId) {
...
io.on("connection", (socket) => {
    ...
    socket.on("createGame", async ({ playerName }, cb) => {
        try {
            const code = makeCode();
            const state = {
                code,
    ...
    // joinGame
    socket.on("joinGame", async ({ code, playerName }, cb) => {
        try {
    ...
    // sendChat
    socket.on("sendChat", async ({ code, message }) => {
    ...
    // playCard
    socket.on("playCard", async ({ code, cardIndex, targetId, guessedCard }) => {
        const g = await fetchGameByCode(code);
        if (!g) {
